...
def get_version_from_wheel(wheel_name):
    ...
    match = re.search(r"newton_usd_schemas-([^-]*)-", wheel_name)
    if match:
...
def is_dev_release(version):
    ...
    return re.search(r"(dev)", version_part)
...
def send_kitmaker_post(version: str, wheel_filename: str):
    ...
    payload = {
        "project_name": "newton-usd-schemas",
        "payload": [
            {
                "pic": kitmaker_pic,
                "job_type": "wheel-release-job",
                "publish_to": "both_devzone_pypi",
                "url": f"{omniverse_pypi_url}/newton-usd-schemas/{version}/{wheel_filename}",
                "size": "small",
                "upload": True,
            }
        ],
    ...
    kitmaker_url = os.environ.get("KITMAKER_UPLOAD_URL")
    if not kitmaker_url:
        raise RuntimeError("KITMAKER_UPLOAD_URL is not set")
    response = requests.post(kitmaker_url, json=payload, headers=headers, verify=False)
...
def main():
    ...
    for wheel_path in wheel_files:
        if wheel_path.is_file():
            ...
            release_status = "preview" if is_dev_release(version) else "ready"

            print(f"  Release Status: {release_status}")

            # Build matrix properties string
            properties = (
                f"component_name=newton-usd-schemas;os={os_name};arch={arch};"
            ...
            jfrog_cmd = [
                ...
                "upload",
                str(wheel_path),
                f"ct-omniverse-pypi/newton-usd-schemas/{version}/",
                "--props",
                properties,
            ]
